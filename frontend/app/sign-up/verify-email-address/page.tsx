      ...
      if (completeSignUp.status === 'complete') {
        ...
        try {
          if (!hasRegistered) {
            ...
            const firstName = (signUp?.unsafeMetadata as Record<string, unknown> | undefined)?.first_name as string | undefined || user?.firstName || payload.firstName || '';
            const lastName = (signUp?.unsafeMetadata as Record<string, unknown> | undefined)?.last_name as string | undefined || user?.lastName || payload.lastName || '';

            if (clerkId && email && firstName && lastName) {
              await registerUser({ clerkId, email, firstName, lastName });
              setHasRegistered(true);
              try { window.localStorage.removeItem('signup_payload'); } catch (_e) { void 0; }
            } else {
              console.error('Missing data to register user in backend', { clerkId, email, firstName, lastName });
            }
          }
        } catch (regErr) {
          console.error('Error registering user in backend after verification:', regErr);
          // On ne bloque pas la navigation, mais on notifie
          toast.error("L'inscription a été vérifiée, mais l'enregistrement interne a échoué. Vous pourrez réessayer plus tard.");
        }
            ...
            try {
              if (!hasRegistered) {
                ...
                if (clerkId && email && firstName && lastName) {
                  ...
                  console.error('Missing data to register user in backend (already verified path)', { clerkId, email, firstName, lastName });
                }
              }
            } catch (regErr2) {
