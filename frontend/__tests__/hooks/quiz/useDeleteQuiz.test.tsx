...
describe('useDeleteQuiz', () => {
  ...
  it('success=true: calls apiClient DELETE, hideLoader, toast.success, invalidateQueries, options.onSuccess', async () => {
    ...
    const { result } = renderHook(() => useDeleteQuiz({ onSuccess }), { wrapper });

    await act(async () => {
      await result.current.deleteQuizAsync({ quizId: 'quiz-1', moduleId: 'module-1' });
    });
    ...
    expect(apiClientMock).toHaveBeenCalledWith('quizzes/quiz-1', 'DELETE', 'token-123');

    expect(toast.success).toHaveBeenCalledWith('Quiz supprime avec succes');
  ...
  it('success=true with status=success: does not invalidate module without moduleId', async () => {
    ...
    await act(async () => {
      await result.current.deleteQuizAsync({ quizId: 'quiz-1b' });
    });
  ...
  it('success=true with null data: still shows success and invalidates module list', async () => {
    ...
    await act(async () => {
      await result.current.deleteQuizAsync({ quizId: 'quiz-1c' });
    });
  ...
  it('success=false: toast.error with message, no invalidateQueries, no onSuccess', async () => {
    ...
    await act(async () => {
      await result.current.deleteQuizAsync({ quizId: 'quiz-2' });
    });

    expect(showLoader).toHaveBeenCalledTimes(1);
    expect(hideLoader).toHaveBeenCalledTimes(1);

    expect(toast.error).toHaveBeenCalledWith('Echec de la suppression du quiz', {
      description: 'Nope',
    });
  ...
  it('success=false without message: toast.error with fallback', async () => {
    ...
    await act(async () => {
      await result.current.deleteQuizAsync({ quizId: 'quiz-3' });
    });
  ...
  it('onError: hideLoader + toast.error with error message', async () => {
    ...
    await act(async () => {
      await expect(result.current.deleteQuizAsync({ quizId: 'quiz-4' })).rejects.toThrow('Boom');
    });
  ...
  it('onError: fallback description when error message is empty', async () => {
    ...
    await act(async () => {
      await expect(result.current.deleteQuizAsync({ quizId: 'quiz-5' })).rejects.toThrow();
    });
