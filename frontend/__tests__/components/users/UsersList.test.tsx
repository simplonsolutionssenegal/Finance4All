...
// Mock child modals
jest.mock('@/components/users/UserInfoModal', () => ({ isOpen, onClose, onDeactivate, user }: any) =>
  isOpen ? (
...
jest.mock('@/components/users/ConfirmDesactivationModal', () => ({ isOpen, onClose, onConfirm }: any) =>
  isOpen ? (
...
jest.mock('@/components/users/RoleEditModal', () => ({ isOpen, onClose }: any) => isOpen ? <div data-testid="role-edit-modal" onClick={onClose}>Role Edit Modal</div> : null);
jest.mock('@/components/users/AddUserModal', () => ({ isOpen, onClose }: any) => isOpen ? <div data-testid="add-user-modal" onClick={onClose}>Add User Modal</div> : null);
...
const mockMemberships = {
  data: [
    { id: 'mem_1', roleName: 'Admin', publicUserData: { userId: 'user_abc', firstName: 'Alice', lastName: 'Smith', identifier: 'alice@example.com' }, createdAt: new Date() },
    { id: 'mem_2', roleName: 'Member', publicUserData: { userId: 'user_def', firstName: 'Bob', lastName: 'Johnson', identifier: 'bob@example.com' }, createdAt: new Date() },
  ],
  isLoading: false,
};
...
const mockInvitations = {
  data: [
      { id: 'inv_1', roleName: 'Member', emailAddress: 'carol@example.com', publicMetadata: { firstName: 'Carol', lastName: 'White' }, createdAt: new Date() }
  ],
  isLoading: false,
};
...
describe('UsersList', () => {
  ...
  it('renders loading state initially', () => {
    mockUseOrganization.mockReturnValue({ organization: { id: 'org_123' }, memberships: { isLoading: true } });
    render(<UsersList />);
  ...
  it('renders "no organization" message when not in an organization', async () => {
    mockUseOrganization.mockReturnValue({ organization: null, isLoaded: true, memberships: { data: [], isLoading: false } });
    mockUseUser.mockReturnValue({ user: { id: 'user_xyz', organizationMemberships: [] }, isLoaded: true });
    render(<UsersList />);
    await waitFor(() => {
  ...
  it('renders "no users" message when there are no members or invitations', async () => {
    mockUseOrganization.mockReturnValue({ organization: { id: 'org_123' }, memberships: { data: [], isLoading: false }, invitations: { data: [], isLoading: false } });
    render(<UsersList />);
    await waitFor(() => {
  ...
  it('renders the list of users and invitations correctly', () => {
    mockUseOrganization.mockReturnValue({ organization: { id: 'org_123' }, memberships: mockMemberships, invitations: mockInvitations });
    render(<UsersList />);
  ...
  it('opens user info modal on remove user click', async () => {
    ...
    mockUseRemoveUserFromOrganization.mockReturnValue({ removeUser: removeUserMock, isLoading: false });
    mockUseOrganization.mockReturnValue({ organization: { id: 'org_123' }, memberships: mockMemberships, invitations: mockInvitations });

    render(<UsersList />);

    await waitFor(() => expect(screen.getAllByTitle("Retirer de l'organisation").length).toBeGreaterThan(0));
