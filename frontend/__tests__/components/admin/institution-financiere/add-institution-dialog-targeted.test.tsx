...
describe('AddInstitutionDialog - Targeted Line Coverage', () => {
  ...
  it('covers toggleRegion functionality (line 171)', async () => {
    ...
    const regionButtons = screen.getAllByRole('button').filter(btn => 
      ['Centrale', 'Littoral', 'Nord', 'Nord-Ouest', 'Ouest', 'Sud', 'Sud-Ouest', 'Adamaoua', 'Est', 'Extrême-Nord'].includes(btn.textContent || '')
    );
    ...
    if (regionButtons.length > 0) {
      ...
      await waitFor(() => {
        const removeButtons = screen.getAllByRole('button').filter(btn => 
          btn.innerHTML.includes('×') || btn.textContent?.includes('×')
        );
