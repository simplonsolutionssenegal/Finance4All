...
describe('AdminDashboardPage', () => {
  it('should render the admin dashboard page correctly', () => {
    ...
    expect(screen.getByText('Tableau de bord administrateur')).toBeInTheDocument();
    expect(screen.getByText('Tableau de bord administrateur')).toHaveClass('text-3xl', 'font-bold', 'mb-6');
  });
  ...
  it('should render with proper responsive grid classes', () => {
    ...
    const gridElement = screen.getByText('Institutions financières').closest('div')?.parentElement?.parentElement;
    expect(gridElement).toHaveClass('grid', 'grid-cols-1', 'md:grid-cols-2', 'lg:grid-cols-4', 'gap-6', 'mb-8');
>>>>>>> 79f75f8 (feat: Add comprehensive test coverage for institution financiere components)
  });
});
