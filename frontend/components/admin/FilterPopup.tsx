...
export interface FilterOptions {
  ...
  status: string[];                       // ex: ['ACTIF','PENDING'] (UPPERCASE à l'apply)
}
...
const FilterPopup: React.FC<FilterPopupProps> = ({ isOpen, onClose, onApplyFilters }) => {
  ...
  const roleOptions = [
    ...
    { value: 'comptable',     label: 'Comptable' }, // (corrigé : "Comptable")
  ];
  ...
  const statusOptions = [
    ...
    // ajoute ici si tu veux: { value: 'inactive', label: 'Inactif' } -> 'INACTIF'
  ];
  ...
  const handleApply = () => {
    ...
    const toBackendStatus = (val: string) => {
      if (val === 'active') return 'ACTIF';
      if (val === 'inactive') return 'INACTIF';
      if (val === 'pending') return 'PENDING';
      return val.toUpperCase();
    };

    const normalised: FilterOptions = {
      role: filters.role.map(r => r.toLowerCase()),      // 'Admin' -> 'admin'
      status: filters.status.map(toBackendStatus),       // 'active' -> 'ACTIF'
      lastConnection: filters.lastConnection,            // 'recent' | 'month' | 'custom'
      customDate: filters.customDate,                    // 'YYYY-MM-DD'
    };

    onApplyFilters(normalised);
    onClose();
  };
