...
// Schéma de validation (custom FileList handling sans any)
const _schema = z.object({
  ...
  // Champs optionnels : on convertit les chaînes vides en undefined
...
export function AddInstitutionDialog({ open, onOpenChange }: Readonly<AddInstitutionDialogProps>) {
  ...
  const resolver: Resolver<InstitutionFormValues> = async values => {
    ...
    const fieldErrors = Object.entries(flat).reduce<Record<string, { type: string; message?: string }>>(
      (acc, [key, messages]) => {
        if (messages && messages.length) acc[key] = { type: 'validation', message: messages[0] };
        return acc;
      },
      {}
    );
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-5'>
              ...
              <div className='flex justify-between pt-5 border-t border-gray-100 mt-6'>
                ...
                {currentStep < totalSteps ? (
                  <Button type='button' onClick={nextStep} className='bg-teal-500 hover:bg-teal-600 text-white font-medium rounded-lg px-5'>
                    Suivant
                  </Button>
                ) : (
