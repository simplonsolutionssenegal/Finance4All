...
const _schema = z.object({
  ...
  contactNom: z.preprocess(
    ...
    z
      .string()
      .min(2, { message: 'Le nom du contact doit contenir au moins 2 caractères.' })
      .optional()
  ),
  ...
  regionsDesservies: z
    .array(z.string())
    .min(1, { message: 'Veuillez sélectionner au moins une région.' }),
});
...
export function AddInstitutionDialog({ open, onOpenChange }: Readonly<AddInstitutionDialogProps>) {
  ...
  const resolver: Resolver<InstitutionFormValues> = async values => {
    ...
    const fieldErrors = Object.entries(flat).reduce<
                              ...
                              return (
                                ...
                                >
                                  <div className='flex items-center'>
                                    ...
                                    <span
                                      className={`ml-2 text-sm ${isSelected ? 'font-medium text-teal-700' : 'text-gray-700'}`}
                                    >
                                      {r.label}
                                    </span>
                                  </div>
