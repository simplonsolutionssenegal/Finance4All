...
import { getLessonStatus } from '@/lib/mocks/learning-mocks';
  ...
  return (
    <div className='space-y-3'>
      {lessons.map(lesson => {
        ...
        if (isDone) buttonLabel = 'Revoir';
        else if (isCurrent) buttonLabel = 'Continuer';
        else buttonLabel = 'Commencer';
          ...
          >
            <CardContent className='flex w-full items-center gap-4 p-4'>
              <div className='flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-grey-100 text-sm font-semibold text-grey-700'>
                {isUnlocked ? lesson.order : <Lock className='h-4 w-4 text-grey-400' />}
              </div>
