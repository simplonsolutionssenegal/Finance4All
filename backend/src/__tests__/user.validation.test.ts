...
describe('User Entity and Use Cases', () => {
  ...
  describe('CreateUserUseCase', () => {
    ...
    it('should throw error when name is missing', async () => {
      ...
      await expect(createUserUseCase.execute('', 'test@example.com'))
        .rejects.toThrow('Le nom et l\'email sont requis');
    });

    it('should throw error when email is missing', async () => {
      const createUserUseCase = new CreateUserUseCaseImpl(mockUserRepository);
      
      await expect(createUserUseCase.execute('Test User', ''))
        .rejects.toThrow('Le nom et l\'email sont requis');
    });
    ...
    it('should throw error when email is invalid', async () => {
      ...
      await expect(createUserUseCase.execute('Test User', 'invalid-email'))
